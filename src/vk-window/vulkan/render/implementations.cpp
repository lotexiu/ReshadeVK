...
bool record_command_buffer(
    ...
    if (beginResult != VK_SUCCESS) {
        std::printf("Erro: nao foi possivel iniciar command buffer (%d)\n", beginResult);
        return false;
    }
    ...
    if (endResult != VK_SUCCESS) {
        std::printf("Erro: nao foi possivel finalizar command buffer (%d)\n", endResult);
        return false;
    }
...
bool create_render_resources(VulkanContext& context) {
    if (!create_render_pass(context)) {
        return false;
    }

    if (!create_framebuffers(context)) {
        return false;
    }

    if (!create_command_resources(context)) {
        return false;
    }

    if (!create_sync_objects(context)) {
        return false;
    }
...
bool draw_frame(VulkanContext& context, const AppConfig& config) {
    ...
    VkResult acquireResult = vkAcquireNextImageKHR(
        ...
        context.swapchain,
        UINT64_MAX,
        context.imageAvailableSemaphores[frame],
        VK_NULL_HANDLE,
        &imageIndex
    );
    ...
    const VkResult submitResult = vkQueueSubmit(context.graphicsQueue, 1, &submitInfo, context.inFlightFences[frame]);
    if (submitResult != VK_SUCCESS) {
        std::printf("Erro: falha no submit da fila grafica (%d)\n", submitResult);
        return false;
    }
