...
static void update_input_glfw(SwapchainData& swData)
{
    ...
    io.AddKeyEvent(ImGuiKey_Enter, canCapture && (
...
static pid_t x11_get_window_pid(Display* display, Window window)
{
    ...
    if (pidAtom == None || window == 0) return -1;
...
void destroy(SwapchainData& swData)
{
    ...
    for (VkSemaphore sem : swData.semaphores)
        if (sem && dispatch.DestroySemaphore) dispatch.DestroySemaphore(device, sem, nullptr);
    swData.semaphores.clear();

    for (VkFence fence : swData.fences)
        if (fence && dispatch.DestroyFence) dispatch.DestroyFence(device, fence, nullptr);
    swData.fences.clear();
    ...
    for (VkFramebuffer fb : swData.framebuffers)
        if (fb && dispatch.DestroyFramebuffer) dispatch.DestroyFramebuffer(device, fb, nullptr);
    swData.framebuffers.clear();

    for (VkImageView iv : swData.imageViews)
        if (iv && dispatch.DestroyImageView) dispatch.DestroyImageView(device, iv, nullptr);
