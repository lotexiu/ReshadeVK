    ...
    VkInstance*                  pInstance)
{
    ...
    if (!gipa || !setInstanceLoaderData) return VK_ERROR_INITIALIZATION_FAILED;
    ...
    VkInstance instance, uint32_t* pPhysicalDeviceCount, VkPhysicalDevice* pPhysicalDevices)
{
    InstanceData* instData = nullptr;
    {
        ...
        if (it == g_instanceData.end()) return VK_ERROR_INITIALIZATION_FAILED;
        instData = &it->second;
    }
    ...
    if ((result == VK_SUCCESS || result == VK_INCOMPLETE) &&
        pPhysicalDevices && pPhysicalDeviceCount && instData->setInstanceLoaderData) {
    ...
    VkInstance instance, const VkAllocationCallbacks* pAllocator)
{
    InstanceDispatch dispatch{};
    {
        std::lock_guard<std::mutex> lock(g_lock);
        auto it = g_instanceData.find(dispatch_key(instance));
        if (it == g_instanceData.end()) return;
    ...
    VkDevice*                    pDevice)
{
    ...
    if (!gipa || !gdpa || !setDeviceLoaderData) return VK_ERROR_INITIALIZATION_FAILED;
    ...
    VkSwapchainKHR*                 pSwapchain)
{
    DeviceData* devData = nullptr;
    {
        std::lock_guard<std::mutex> lock(g_lock);
        auto it = g_deviceData.find(dispatch_key(device));
        if (it == g_deviceData.end()) return VK_ERROR_INITIALIZATION_FAILED;
        devData = &it->second;
    }
...
{
#define HOOK(vkName, fn) if (!std::strcmp(name, #vkName)) return reinterpret_cast<PFN_vkVoidFunction>(fn);
    ...
    VkInstance instance, const char* name)
{
    ...
    if (it == reshadevk::g_instanceData.end()) return nullptr;
    ...
    VkDevice device, const char* name)
{
    ...
    if (it == reshadevk::g_deviceData.end()) return nullptr;
